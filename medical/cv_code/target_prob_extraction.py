...
def main():
    ...
    args.data_dir=path.join(args.data_dir, args.model, str(args.seed))
    ...
    prob_generator = GenerateNextTokenProbAPI(client, args.endpoint_name)
    ...
    for name, samples in train_test_ents.items():
        for j, ent_list in tqdm(enumerate(samples[:2])):
            print(f'{name.upper()}: {j+1}/{len(samples[:2])}...')
    ...
    if ablation_pct != 1.0:
        ...
        with open(path.join(args.save_dir, f'{args.model}_target_probs_prompt_{PROMPT_TO_USE}_{ablation_str}.json'), 'w') as f:
            json.dump(results, f)
    else:
        with open(path.join(args.save_dir, f'{args.model}_target_probs_prompt_{PROMPT_TO_USE}.json'), 'w') as f:
            json.dump(results, f)
