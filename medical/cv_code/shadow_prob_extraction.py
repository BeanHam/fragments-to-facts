...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='cv_probs/')
    parser.add_argument('--model', type=str, default='llama')
    parser.add_argument('--endpoint_name', type=str, default='endpoint_names')
    parser.add_argument('--seed', type=int, default=0)
    parser.add_argument('--ablation_pct', type=str, default='1.0')
    parser.add_argument('--together_key', type=str)
    args = parser.parse_args()
    
    args.save_dir=path.join(args.save_dir, args.model, str(args.seed))
    client = Together(api_key=args.together_key)
    shadow_model_endpoints = [args.endpoint_name]

    ablation_str = int(float(args.ablation_pct)*100)
    if ablation_str != 100:
        with open(path.join(args.save_dir, f'{args.model}_target_probs_prompt_{PROMPT_TO_USE}_{ablation_str}.json'), 'r') as f:
            all_probs = json.load(f)
    else:
        with open(path.join(args.save_dir, f'{args.model}_target_probs_prompt_{PROMPT_TO_USE}.json'), 'r') as f:
            all_probs = json.load(f)
    ...
    print('Save Results...')
    if ablation_str != 100:
        with open(path.join(args.save_dir, f'{args.model}_shadow_probs_prompt_{PROMPT_TO_USE}_{ablation_str}.json'), 'w') as f:
            json.dump(all_probs, f)
    else:
        with open(path.join(args.save_dir, f'{args.model}_shadow_probs_prompt_{PROMPT_TO_USE}.json'), 'w') as f:
            json.dump(all_probs, f)
