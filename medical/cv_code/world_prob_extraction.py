...
def main():
    ...
    if ablation_str != 100:
        with open(path.join(args.save_dir, f'{args.model}_shadow_probs_prompt_{PROMPT_TO_USE}_{ablation_str}.json'), 'r') as f:
            all_probs = json.load(f)
    else:
        with open(path.join(args.save_dir, f'{args.model}_shadow_probs_prompt_{PROMPT_TO_USE}.json'), 'r') as f:
            all_probs = json.load(f)
    ...
    print('Save Results...')
    if ablation_str != 100:
        with open(path.join(args.save_dir, f'{args.model}_world_probs_prompt_{PROMPT_TO_USE}_{ablation_str}.json'), 'w') as f:
            json.dump(all_probs, f)
    else:
        with open(path.join(args.save_dir, f'{args.model}_world_probs_prompt_{PROMPT_TO_USE}.json'), 'w') as f:
            json.dump(all_probs, f)
